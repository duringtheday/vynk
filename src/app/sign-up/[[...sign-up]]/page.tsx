...
const C = {
  ...
  silver:'#BFC3C9', smoke:'#6F737A', nd:'#08090B', nl:'#141720',
}
...
export default function SignUpPage() {
  ...
  async function applyPromo() {
    ...
    try {
      ...
      if (data.valid) {
        ...
        setPromoMsg(
          data.discountType === 'percent' ? `${data.discountValue}% de descuento aplicado` :
          data.discountType === 'fixed'   ? `$${(data.discountValue/100).toFixed(2)} de descuento aplicado` :
      ...
      setPromoStatus('invalid')
      setPromoMsg('Error de red. Intenta de nuevo.')
    }
  }
  ...
  return (
    <main style={{ minHeight:'100dvh', background:C.g, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-start', padding:'32px 20px 80px', fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <p style={{ color:C.smoke, fontSize:'13px', marginBottom:'24px', fontWeight:300 }}>Create your digital identity</p>
      ...
      <div style={{ width:'100%', maxWidth:'400px', background:C.g, boxShadow:`10px 10px 28px ${C.nd}, -6px -6px 18px ${C.nl}`, borderRadius:'24px', border:'1px solid rgba(212,168,79,0.06)', overflow:'hidden' }}>
        ...
        {!showClerk && (
          <div style={{ padding:'24px 24px 0' }}>
            <p style={{ fontSize:'11px', color:C.smoke, marginBottom:'8px', fontWeight:600, letterSpacing:'.06em', textTransform:'uppercase' }}>Promo code</p>
            <div style={{ display:'flex', gap:'8px', marginBottom:'10px' }}>
              <input
                value={promoCode}
                onChange={e => { setPromoCode(e.target.value.toUpperCase()); setPromoStatus('idle'); setPromoMsg('') }}
                onKeyDown={e => e.key==='Enter' && applyPromo()}
                placeholder="VYNK2025"
                maxLength={32}
            ...
            {hidePhone&&<div style={{ marginBottom:'14px', padding:'9px 13px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'10px', fontSize:'12px', color:C.smoke, display:'flex', alignItems:'center', gap:'8px' }}><span>📵</span><span>Phone number not required for this registration</span></div>}
            {optionalPhone&&<div style={{ marginBottom:'14px', padding:'9px 13px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'10px', fontSize:'12px', color:C.smoke, display:'flex', alignItems:'center', gap:'8px' }}><span>📱</span><span>Phone number is optional</span></div>}

            <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
              <div style={{ flex:1, height:'1px', background:'rgba(255,255,255,0.05)' }}/>
              <span style={{ fontSize:'11px', color:C.smoke }}>or skip</span>
              <div style={{ flex:1, height:'1px', background:'rgba(255,255,255,0.05)' }}/>
            </div>
          </div>
        )}
        ...
        {!showClerk ? (
          <div style={{ padding:'16px 24px 28px' }}>
            ...
            <p style={{ textAlign:'center', fontSize:'11px', color:C.smoke, marginTop:'12px' }}>
              ...
              <Link href="/sign-in" style={{ color:C.gold, textDecoration:'none' }}>Sign in</Link>
            </p>
          </div>
        ) : (
