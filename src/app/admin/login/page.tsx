...
export default function AdminLoginPage() {
  ...
  return (
    <main style={{ minHeight:'100dvh', background:C.g, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px', fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ width:'320px', background:C.g, boxShadow:`14px 14px 36px ${C.nd}, -8px -8px 24px ${C.nl}`, borderRadius:'32px', padding:'40px 32px', border:'1px solid rgba(212,168,79,0.06)', textAlign:'center' }}>
        <Image src="/logo.png" alt="Vynk" width={90} height={29} style={{ objectFit:'contain', marginBottom:'28px', opacity:.8 }} />
        <div style={{ width:'60px', height:'60px', background:C.g, boxShadow:raisedSm, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 18px', fontSize:'24px' }}>🔐</div>
        <div style={{ color:C.gold, fontSize:'15px', fontWeight:700, marginBottom:'4px', fontFamily:"'Syne',sans-serif" }}>Owner Vault</div>
        <div style={{ color:C.smoke, fontSize:'12px', marginBottom:'28px', fontWeight:300 }}>
          {step==='pin' ? 'Enter your 6-digit PIN' : 'Enter the code sent to your email'}
        </div>
        <div style={{ display:'flex', justifyContent:'center', gap:'6px', marginBottom:'24px' }}>
          {[0,1,2].map(i=>(
            <div key={i} style={{ height:'3px', borderRadius:'2px', transition:'all .3s', width: i<=(step==='2fa'?1:0)?'28px':'6px', background: i<=(step==='2fa'?1:0)?C.gold:C.smoke, opacity: i<=(step==='2fa'?1:0)?1:.3 }} />
          ))}
        </div>
          ...
          <>
            <div style={{ display:'flex', justifyContent:'center', gap:'8px', marginBottom:'24px' }}>
              {Array.from({length:6}).map((_,i)=>(
                <div key={i} style={{ width:'38px', height:'46px', borderRadius:'10px', background:C.g, boxShadow: i<pin.length?`inset 3px 3px 8px ${C.nd}, inset -2px -2px 6px rgba(212,168,79,0.1), 0 0 8px rgba(212,168,79,0.12)`:insetSm, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'18px', color:C.gold, fontWeight:700, transition:'all .15s', border: i<pin.length?'1px solid rgba(212,168,79,0.12)':'1px solid rgba(255,255,255,0.02)' }}>
                  {i<pin.length?'●':''}
                </div>
              ))}
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px', marginBottom:'16px' }}>
              {digits.map((d,i)=>(
                <button key={i} onClick={()=>{ if(d==='⌫') delDigit(); else if(d) addDigit(d) }}
                  ...
                  style={{ height:'52px', borderRadius:'12px', background:C.g, boxShadow:d?raisedSm:'none', border:'1px solid rgba(255,255,255,0.025)', color:d==='⌫'?C.gold:C.silver, fontSize:'18px', fontWeight:600, cursor:d?'pointer':'default', fontFamily:"'DM Sans',sans-serif", visibility:d?'visible':'hidden', outline:'none', transition:'all .1s' }}>
                  {d}
                </button>
              ))}
            </div>
            ...
            {attempts>0&&<p style={{ color:'#ef4444', fontSize:'12px', marginTop:'10px' }}>{3-attempts} attempt(s) remaining before lockout</p>}
          </>
        ) : (
          <>
            <input type="text" maxLength={6} placeholder="000000" value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,''))} autoFocus
              style={{ width:'100%', textAlign:'center', fontSize:'32px', fontWeight:700, letterSpacing:'.4em', background:C.g, border:'1px solid rgba(212,168,79,0.1)', color:C.silver, borderRadius:'14px', padding:'16px 8px', marginBottom:'16px', fontFamily:"'DM Sans',sans-serif", outline:'none', boxShadow:inset }} />
