...
export async function GET() {
  ...
  if (!userId) {
    return NextResponse.json({ card: null, isOwner: false })
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!userId) {
    return NextResponse.json(
      {
        error: isDev
          ? 'Auth disabled in development. POST /api/cards requires Clerk in production.'
          : 'Unauthorized',
      },
      { status: 401 }
    )
  ...
  if (body.promoCode) {
    const [promo] = await db
      .select()
      .from(promoCodes)
      .where(eq(promoCodes.code, body.promoCode.toUpperCase()))
      .limit(1)
    ...
    if (promo && promo.isActive) {
      ...
      if (!expired && !maxed && promo.discountType === 'free') {
        ...
        await db
          .update(promoCodes)
          .set({ usesCount: (promo.usesCount || 0) + 1 })
