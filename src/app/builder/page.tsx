...
export default function BuilderPage() {
  ...
  function getPoint(
    ...
    const m = e as MouseEvent | React.MouseEvent
  ...
  function startPhotoInnerMode(e: React.MouseEvent | React.TouchEvent) {
    ...
    photoInnerDrag.current = { mx: p.x, my: p.y, ox: photoObjPos.x, oy: photoObjPos.y }
  }
  ...
  function handlePhotoWheel(e: React.WheelEvent) {
    ...
    if (e.shiftKey || e.altKey) {
      setPhotoRotate(r => Number((r + e.deltaY * 0.18).toFixed(2)))
      return
    }

    const next = Math.max(0.3, Math.min(5, photoScale + e.deltaY * -0.0025))
    setPhotoScale(Number(next.toFixed(3)))
  }

  useEffect(() => {
    function onMove(e: MouseEvent | TouchEvent) {
      if (photoInnerGesture.current && 'touches' in e && e.touches.length >= 2) {
        if ('cancelable' in e && e.cancelable) e.preventDefault()
        const info = getTouchesInfo(e.touches)
        const nextScale = Math.max(0.3, Math.min(5, photoInnerGesture.current.startScale * (info.dist / photoInnerGesture.current.dist)))
        const nextRotate = photoInnerGesture.current.startRotate + (info.angle - photoInnerGesture.current.angle)
      ...
      const p = getPoint(e)
      const dx = p.x - canvasDrag.current.mx
      const dy = p.y - canvasDrag.current.my
      ...
      setCardOffset({
        x: canvasDrag.current.ox + dx,
        y: canvasDrag.current.oy + dy,
      })
    }
          ...
          <div style={{ perspective: '800px', flex: 1 }}>
            <div
              ...
              style={{
                ...
                transition: dragging.current || canvasDrag.current ? 'none' : 'transform .7s cubic-bezier(0.23,1,0.32,1)',
