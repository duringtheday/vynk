...
export async function verifyPin(pin: string, hash?: string): Promise<boolean> {
  const h = (hash || process.env.ADMIN_PIN_HASH || '').trim()
  if (!h) return false
  return bcrypt.compare(pin, h)
}
...
export async function send2FAEmail(code: string) {
  // Use onboarding@resend.dev until custom domain is verified in Resend
  const from = process.env.RESEND_FROM_EMAIL || 'onboarding@resend.dev'
  await resend.emails.send({
    from,
    to:      process.env.OWNER_EMAIL!,
    subject: `Vynk Admin — Your verification code: ${code}`,
    html: `
      <div style="font-family:sans-serif;max-width:400px;margin:0 auto;padding:32px;background:#0D0F12;border-radius:16px;color:#fff">
        ...
        <p style="color:#6F737A;font-size:12px">Expires in 5 minutes. If you didn't request this, ignore this email.</p>
      </div>
    `,
  })
}
...
export async function logAccess(event: { type:string; ip?:string; country?:string; device?:string; browser?:string; notes?:string }) {
  try {
    await db.insert(adminLog).values({
      event: event.type, ip: event.ip, country: event.country,
      device: event.device, browser: event.browser, notes: event.notes,
    })
  } catch {}
}
