...
const C = { g:'#0D0F12', gold:'#D4A84F', smoke:'#6F737A', nd:'#08090B', nl:'#141720' }

function CardFront() {
  return (
    <div style={{ width:'300px', height:'185px', borderRadius:'18px', background:'linear-gradient(135deg,#0D0F12 0%,#141720 100%)', border:'1px solid rgba(212,168,79,0.2)', boxShadow:'0 32px 64px rgba(0,0,0,0.8),0 0 0 1px rgba(212,168,79,0.08),inset 0 1px 0 rgba(212,168,79,0.12)', position:'relative', overflow:'hidden', padding:'22px', display:'flex', flexDirection:'column', justifyContent:'space-between', color:'#BFC3C9', fontFamily:"'DM Sans',sans-serif", flexShrink:0 }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', position:'relative', zIndex:1 }}>
        ...
        {/* FIX #8: logo fills the circle, no objectFit contain, no border radius on img */}
        <div style={{ width:'46px', height:'46px', borderRadius:'50%', border:'2px solid rgba(212,168,79,0.3)', background:'#0a0b0f', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 12px rgba(0,0,0,0.6)', overflow:'hidden', flexShrink:0 }}>
          <img src="/logo.png" alt="Vynk" style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }}/>
        </div>
      </div>
      ...
      <div style={{ position:'absolute', bottom:'18px', right:'20px', width:'34px', height:'34px', border:'1px solid rgba(212,168,79,0.2)', borderRadius:'6px', background:'rgba(212,168,79,0.04)', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'2px', padding:'4px' }}>
...
function CardBack() {
  ...
  return (
    <div style={{ width:'300px', height:'185px', borderRadius:'18px', background:'linear-gradient(135deg,#0a0b0f 0%,#111420 100%)', border:'1px solid rgba(212,168,79,0.15)', boxShadow:'0 32px 64px rgba(0,0,0,0.8),0 0 0 1px rgba(212,168,79,0.06),inset 0 1px 0 rgba(212,168,79,0.08)', position:'relative', overflow:'hidden', padding:'22px', display:'flex', flexDirection:'column', justifyContent:'space-between', color:'#BFC3C9', fontFamily:"'DM Sans',sans-serif", flexShrink:0 }}>
      <svg style={{ position:'absolute', inset:0, width:'100%', height:'100%', pointerEvents:'none' }} viewBox="0 0 300 185" preserveAspectRatio="xMidYMid slice">
  ...
  if (isMobile) {
    return (
      <div style={{ width:'100%', display:'flex', flexDirection:'column', alignItems:'center', gap:'16px', padding:'8px 0 24px' }}>
        ...
        <div style={{ transform:`translateY(${floatFront}px) rotate(${rotateFront}deg)` }}><CardFront/></div>
        <div style={{ fontSize:'10px', color:C.smoke, letterSpacing:'.08em', textTransform:'uppercase', fontWeight:600, marginTop:'4px' }}>Back</div>
        <div style={{ transform:`translateY(${floatBack}px) rotate(${-rotateBack}deg)`, opacity:.9 }}><CardBack/></div>
      </div>
    )
  }

  return (
    <div style={{ position:'relative', width:'100%', height:'460px', display:'flex', alignItems:'flex-start', justifyContent:'center', paddingTop:'20px' }}>
      <div style={{ position:'absolute', width:'380px', height:'180px', borderRadius:'50%', background:'radial-gradient(ellipse,rgba(212,168,79,0.1) 0%,transparent 70%)', top:'50%', left:'50%', transform:'translate(-50%,-50%)', pointerEvents:'none', filter:'blur(40px)' }}/>

      <div style={{ position:'absolute', top:'10px', left:'50%', marginLeft:'-170px', transform:`translateY(${floatFront}px) rotate(${-4+rotateFront}deg)`, zIndex:2 }}>
        <CardFront/>
      ...
      <div style={{ position:'absolute', top:'220px', left:'50%', marginLeft:'-80px', transform:`translateY(${floatBack}px) rotate(${6+rotateBack}deg)`, zIndex:1, opacity:.92 }}>
        <CardBack/>
